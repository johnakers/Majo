...
HXLINE( 523)				_hx_tmp = (encodedData != HX_("",00,00,00,00));
            			}
            			else {
...
HXLINE( 530)					sharedObject->data = unserializer->unserialize();
            					...
            					if (_hx_e.IsClass<  ::Dynamic >() ){
            						HX_STACK_BEGIN_CATCH
            						 ::Dynamic _g = _hx_e;
HXLINE( 532)						 ::haxe::Exception e = ::haxe::Exception_obj::caught(_g);
HXLINE( 534)						::flixel::util::_FlxSave::FlxSharedObject_obj::all->set(id,sharedObject);
HXLINE( 535)						return ::flixel::util::LoadResult_obj::FAILURE(::flixel::util::LoadFailureType_obj::PARSING(encodedData,e),sharedObject);
            					}
            					else {
            						HX_STACK_DO_THROW(_hx_e);
            					}
